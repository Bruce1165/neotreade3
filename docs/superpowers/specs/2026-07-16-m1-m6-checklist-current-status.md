...
- [x] 决策可审计（至少包含输入引用与关键派生/中间状态的定位线索）
  ...
  - 证据：聚合 list 入口（按 run_id/report_id 过滤；_meta 回显 run_id + matched_count）：[main.py:L3338-L3405](file:///Users/mac/NeoTrade3/apps/api/main.py#L3338-L3405)
  - 证据：聚合 list 单测（run_id 过滤返回；_meta.matched_count/returned_count 区分；invalid_run_id 400）：[test_m3_lifecycle_log_api_readback.py:L89-L270](file:///Users/mac/NeoTrade3/tests/unit/test_m3_lifecycle_log_api_readback.py#L89-L270)
