...
> 归档说明：本文为 2026-06-02 的历史设计规格，保留当时自动化链路与任务命名证据。文中将 `update_daily_prices_tencent` 视为正式触发器的表述不代表当前正式生产口径；当前正式日线任务名为 `update_daily_prices_authoritative`。
...
5. 团队主题数据源：使用独立 provider 抽象与最近成功快照；东方财富接口已退役，不再作为当前主源。  
...
实现为可插拔 provider：
- Provider 1：当前主题主源 provider（不使用东方财富接口）  
