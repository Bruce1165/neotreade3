...
describe('App route guard', () => {
  ...
  it('shows not found guard on unknown route', async () => {
    ...
    expect(screen.getByText('路径无效，请返回已登记的工作台入口。')).toBeTruthy()
    expect(screen.getByText('返回选股工作台')).toBeTruthy()
    expect(screen.getByText('团队控制台：审阅 / 监控 / 复盘')).toBeTruthy()
  })

  it('shows not found guard on missing report id route', async () => {
    window.history.pushState({}, '', '/lowfreq/backtest-reports/')
    ...
    expect(screen.getByText('路径无效，请返回已登记的工作台入口。')).toBeTruthy()
    expect(screen.queryByText('LowfreqBacktestReport Page')).toBeNull()
  })
})
