...
def test_m3_lifecycle_logs_list_endpoint_returns_400_for_invalid_run_id(
    ...
    with pytest.raises(ApiError) as exc:
        router.dispatch(f"/api/m3/lifecycle-logs?run_id=..&cursor={cursor}")
...
def test_m3_lifecycle_logs_list_endpoint_run_id_blank_cursor_offset_error_precedence(
    tmp_path: Path,
) -> None:
    _write_lifecycle_log_fixtures(
        project_root=tmp_path,
    ...
    with pytest.raises(ApiError) as exc:
        router.dispatch(f"/api/m3/lifecycle-logs?run_id=%20&limit=1&cursor={cursor}")

    assert exc.value.status_code == HTTPStatus.BAD_REQUEST
    assert exc.value.code == "invalid_run_id"

    with pytest.raises(ApiError) as exc:
        router.dispatch(f"/api/m3/lifecycle-logs?run_id=%20&limit=1&offset=1&cursor={cursor}")

    assert exc.value.status_code == HTTPStatus.BAD_REQUEST
    assert exc.value.code == "invalid_pagination"

    with pytest.raises(ApiError) as exc:
        router.dispatch("/api/m3/lifecycle-logs?run_id=%20&limit=1&offset=1&cursor=abc")

    assert exc.value.status_code == HTTPStatus.BAD_REQUEST
    assert exc.value.code == "invalid_cursor"


def test_m3_lifecycle_logs_list_endpoint_run_id_empty_string_cursor_offset_error_precedence(
    tmp_path: Path,
) -> None:
    _write_lifecycle_log_fixtures(
        project_root=tmp_path,
    ...
    with pytest.raises(ApiError) as exc:
        router.dispatch(f"/api/m3/lifecycle-logs?run_id=&limit=1&cursor={cursor}")

    assert exc.value.status_code == HTTPStatus.BAD_REQUEST
    assert exc.value.code == "invalid_run_id"

    with pytest.raises(ApiError) as exc:
        router.dispatch(f"/api/m3/lifecycle-logs?run_id=&limit=1&offset=1&cursor={cursor}")

    assert exc.value.status_code == HTTPStatus.BAD_REQUEST
    assert exc.value.code == "invalid_pagination"

    with pytest.raises(ApiError) as exc:
        router.dispatch("/api/m3/lifecycle-logs?run_id=&limit=1&offset=1&cursor=abc")
...
def test_m3_lifecycle_logs_list_endpoint_returns_400_for_cursor_offset_conflict(
    ...
    with pytest.raises(ApiError) as exc:
        router.dispatch(
            f"/api/m3/lifecycle-logs?run_id=2026-06-20&limit=1&offset=1&cursor=abc"
        )
