...
def build_hot_sector_signal_seed(candidate: Any, *, market_filter_note: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "wave_phase_confidence": float(getattr(candidate, "wave_phase_confidence", 0.0) or 0.0),
        "role": candidate.role,
        "reasons": reasons,
        "evidence_bundle": list(getattr(candidate, "evidence_bundle", []) or reasons),
        "pattern_evidence": list(getattr(candidate, "pattern_evidence", []) or []),
        "pe": candidate.pe_ttm,
