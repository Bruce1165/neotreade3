#!/usr/bin/env python3
"""
低频量化交易引擎 v5 - Auto Research Round 3 优化版

Round 2 教训：
- 20日涨幅>15%的过滤过于严格，反而效果更差
- 需要回到v3的基础，但优化波浪判断逻辑

Round 3 优化方向：
1. 取消20日涨幅硬性过滤
2. 优化3浪判断：重点看价格突破+均线多头排列+温和放量
3. 增加"杯柄形态"检测作为加分项
4. 止损保持-10%，但增加"反弹确认"机制
"""
...
class LowFreqTradingEngineV5:
    """低频量化交易引擎 v5 - Round 3优化"""
    ...
    def detect_cup_handle(self, code: str, target_date: date) -> float:
        ...
        if len(vols) >= 15:
            ...
            if pre_vol > 0 and recent_vol / pre_vol < 0.9:  # 缩量
                score += 0.3
        
        # 3. 检查当前是否接近近期高点（突破前）
        recent_high = max(closes[-20:])
        current = closes[-1]
        if recent_high > 0 and current / recent_high > 0.95:  # 接近高点
            score += 0.4
        
        return min(score, 1.0)

    def detect_wave_phase_v5(self, code: str, target_date: date) -> tuple[str, float]:
        """
        Round 3优化：简化波浪判断，重点看趋势强度
        """
        conn = self._conn()
        cursor = conn.cursor()
        ...
        recent_low_40 = min(closes[:-5]) if len(closes) > 5 else min(closes)
        ...
        # 价格接近近期高点
        if recent_high_40 > 0:
            price_to_high = current_price / recent_high_40
            if price_to_high >= 0.98:
                wave3_score += 25
            elif price_to_high >= 0.95:
                wave3_score += 15
        ...
        if ma5 > ma10:  # 短期多头
            wave1_score += 25
        if 1.0 < vol_ratio < 2.0:  # 温和放量
            wave1_score += 25
        if current_price > ma20:  # 站上20日线
            wave1_score += 25
    ...
    def get_sector_candidates(self, sector: str, target_date: date, top_n: int = 3) -> list[StockCandidate]:
        ...
        for i, (code, name, mkt_cap, close, pct_chg, amount, volume) in enumerate(rows[:15]):
            ...
            # 波浪判断
            wave_phase, wave_confidence = self.detect_wave_phase_v5(code, target_date)
            ...
            elif wave_phase == WavePhase.WAVE_3.value:
                score += 20
                reasons.append(f"3浪(置信{wave_confidence:.0%})")
            ...
            try:
                ...
                for c in candidates:
                    ...
                    if c.buy_score >= self.BUY_THRESHOLD and c.role == "龙头" and wave_ok:
                        ...
                        logger.info(f"  买入信号: {c.code} {c.name} | 评分:{c.buy_score} | {c.wave_phase} | 杯柄:{c.cup_handle_score:.1f}")
    ...
    def run_backtest(self, start_date: date, end_date: date,
        ...
        """运行回测"""
        logger.info(f"低频交易回测 v5: {start_date} ~ {end_date}")
                    ...
                    for sig in signals["buy_signals"]:
                        ...
                        if shares >= 100 and shares * price <= capital:
                            ...
                            positions[sig["code"]] = TradeRecord(
                                ...
                                cup_handle_score=sig["cup_handle_score"],
                                status="open")
...
def main():
    engine = LowFreqTradingEngineV5()
    start_date = date(2024, 11, 26)
    end_date = date(2026, 5, 22)

    print(f"\n{'='*70}")
    print(f"低频量化交易系统 v5 (Round 3优化: 简化波浪判断+杯柄检测)")
    print(f"{'='*70}")
    print(f"回测区间: {start_date} ~ {end_date}")
    print(f"买入条件: 评分≥80 + 龙头 + 3浪/1浪")
    print(f"新增: 杯柄形态检测")
    print(f"止损线: -10%")
    print(f"{'='*70}\n")
    ...
    with open(output_dir / f"lowfreq_v5_{start_date.isoformat()}_{end_date.isoformat()}.json", 'w') as f:
