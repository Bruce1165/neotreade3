...
class TradeRecord:
    ...
    shares: int = 0
    shares_sold: int = 0  # 已止盈数量（新增）
    hold_days: int = 0
    return_pct: float = 0.0
    buy_score: float = 0.0
    wave_phase: str = ""
    peak_price: float = 0.0  # 持仓期间最高价（新增）
    partial_taken: bool = False  # 是否已分批止盈（新增）
    sell_reason: str = ""
    status: str = "open"
...
class LowFreqTradingEngineV3:
    ...
    # ===== 参数配置（基于反推法优化） =====
    MARKET_CAP_MAX = 400e8           # 最大市值400亿
    MARKET_CAP_MIN = 200e8           # 最小市值200亿
    BUY_THRESHOLD = 80               # 买入确定性阈值
    TARGET_RETURN = 30.0             # 目标收益率30%
    PARTIAL_PROFIT_LEVEL = 30.0      # 分批止盈线（v14: 从25提高到30，目标达成即全出）
    PARTIAL_PROFIT_PCT = 100         # 分批止盈比例（v14: 从50改为100，全出）
    TRAILING_PROFIT_LEVEL = 20.0     # 触发尾随止损的盈利水平
    ...
    ANTI_REPEAT_DAYS = 45            # v14新增：防重复入场冷却期
    ...
    def check_sell_signal_v2(self, trade: TradeRecord, current_date: date) -> Optional[SellSignal]:
        """检查是否触发卖出信号 - 包含分批止盈和尾随止损"""
        sell_price = self._get_price(trade.code, current_date)
        if not sell_price or sell_price <= 0:
            return None

        ret_pct = (sell_price - trade.buy_price) / trade.buy_price * 100
        ...
        if ret_pct >= self.TARGET_RETURN:
            return SellSignal(
                ...
                confidence=95,
                details=f"目标达成: {ret_pct:.1f}% (持有{hold_days}天)"
            )
        ...
        if ret_pct <= current_stop:
            return SellSignal(
                reason="stop_loss",
                confidence=90,
                details=f"止损: {ret_pct:.1f}% (线:{current_stop:.0f}%, 持有{hold_days}天)"
            )
        ...
        if hold_days >= 20 and amt_ratio_today < 0.5:
            return SellSignal(
                ...
                confidence=85,
                details=f"人气消散: {amt_ratio_today:.2f}<0.5倍, {hold_days}天"
            )
    ...
    def check_sell_signal(self, code: str, buy_date: date, buy_price: float,
        ...
        return self.check_sell_signal_v2(TradeRecord(
            code=code, name="", sector="", buy_date=buy_date.isoformat(),
            buy_price=buy_price, shares=0, buy_score=buy_score,
            peak_price=buy_price, status="open"), current_date)
    ...
    def run_backtest(
        ...
        """运行完整回测 - 包含分批止盈和尾随止损"""
        logger.info(f"低频交易回测 Final: {start_date} ~ {end_date}")
        ...
        daily_values = []
        # v13: 记录最近卖出的股票及日期，用于防重复入场
        recently_sold: dict[str, date] = {}
        ...
        for i, current_date in enumerate(trading_dates):
            ...
            for code, trade in list(positions.items()):
                ...
                if sell:
                    ...
                    if sell.reason == "partial_profit" and not trade.partial_taken:
                        # v14: 根据PARTIAL_PROFIT_PCT决定卖出比例
                        if self.PARTIAL_PROFIT_PCT >= 100:
                            # 全出模式
                            profit_shares = trade.shares
                            sell_pct_str = "100%"
                        else:
                            # 分批模式
                            profit_shares = int(trade.shares * self.PARTIAL_PROFIT_PCT / 100 / 100) * 100
                            sell_pct_str = f"{self.PARTIAL_PROFIT_PCT}%"
                        ...
                        if profit_shares > 0:
                            ...
                            partial_trade = TradeRecord(
                                ...
                                hold_days=self._count_trading_days(date.fromisoformat(trade.buy_date), current_date),
                                sell_reason=f"止盈{sell_pct_str}",
                                status="closed")
                            all_trades.append(partial_trade)
                            logger.info(f"  止盈{sell_pct_str}: {code} 卖出{profit_shares}股 当前盈利{partial_ret:.1f}%")
                            # v14: 如果是全出，不再继续观察
                            if self.PARTIAL_PROFIT_PCT >= 100:
            ...
            for code in closed_codes:
                del positions[code]
                # v13: 记录卖出的股票，加入冷却期
                recently_sold[code] = current_date
                # 清理过期的冷却记录
                expired = [c for c, d in recently_sold.items()
                          if (current_date - d).days > self.ANTI_REPEAT_DAYS]
                for c in expired:
                    del recently_sold[c]
                ...
                try:
                    ...
                    for sig in signals["buy_signals"]:
                        if sig["code"] in positions:
                            continue
                        # v13: 检查是否在冷却期内
                        if sig["code"] in recently_sold:
                            days_since_sold = (current_date - recently_sold[sig["code"]]).days
                            logger.info(f"  跳过: {sig['code']} {sig['name']} (卖出冷却期剩余{self.ANTI_REPEAT_DAYS - days_since_sold}天)")
                            continue
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"低频量化交易系统 Final (v14: 30%全出止盈+45天冷却期)")
    print(f"{'='*70}")
    ...
    print(f"分批止盈: 盈利>{engine.PARTIAL_PROFIT_LEVEL}%时卖出{int(engine.PARTIAL_PROFIT_PCT)}%仓位")
    print(f"{'='*70}\n")
    ...
    if result['recent_trades']:
        print(f"\n最近交易记录:")
        for t in result['recent_trades'][-15:]:
            print(f"  {t['code']} {t['name']} | {t['buy_date']}→{t['sell_date']} | "
                  f"{t['hold_days']}天 | {t['return_pct']:+.1f}%")

    # 保存结果
    import json
    output_dir = Path("var/backtest_results")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"lowfreq_final_{start_date.isoformat()}_{end_date.isoformat()}.json"
