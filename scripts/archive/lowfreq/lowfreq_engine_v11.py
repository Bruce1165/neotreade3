...
v10教训：
- 趋势破位条件太激进，触发11次
- 分批止盈效果不错（300972万辰集团案例）

v11策略：
1. 基于v3基础（最稳定）
2. 只保留有效的优化：
   - 分批止盈：盈利>25%时卖出50%
   - 尾随止损：盈利>20%后，止损提高到-5%
3. 移除所有激进的卖出条件
4. 持有期放宽到15-60天
"""
...
class LowFreqTradingEngineV11:
    ...
    TRAILING_PROFIT_LEVEL = 20.0  # 触发尾随止损
    TRAILING_STOP_PCT = -5.0     # 尾随止损线
    MIN_HOLD_DAYS = 15            # 最小持仓（放宽）
    MAX_HOLD_DAYS = 60
    ...
    def get_sector_candidates(self, sector: str, target_date: date, top_n: int = 3) -> list[dict]:
        ...
        for i, (code, name, mkt_cap, close, pct_chg) in enumerate(rows[:10]):
            ...
            avg_vol = np.mean([h[1] for h in history[1:6] if h[1] > 0]) if len(history) >= 6 else 1
            vol_ratio = history[0][1] / avg_vol if avg_vol > 0 and history[0][1] else 1.0
    ...
    def check_sell_signal(self, trade: TradeRecord, current_date: date, 
                          current_price: float) -> tuple[bool, str, str]:
        """只保留核心卖出条件"""
        ...
        # 5. 弱势保护（30天后涨幅<5%则卖出）
        if hold_days >= 30 and ret_pct < 5:
            return True, "weak_hold", f"弱势: {hold_days}天仅{ret_pct:.1f}%"
        
        return False, "", ""

    def run_backtest(self, start_date: date, end_date: date,
                     initial_capital: float = 1000000.0) -> dict:
        logger.info(f"低频交易回测 v11: {start_date} ~ {end_date}")
        ...
        for i, current_date in enumerate(trading_dates):
            ...
            for code, trade in list(positions.items()):
                ...
                should_sell, reason, detail = self.check_sell_signal(trade, current_date, current_price)
                ...
                if should_sell:
                    if reason == "partial_profit" and not trade.partial_taken:
                        ...
                        logger.info(f"  卖出: {trade.code} | {reason} | {ret:+.1f}% | {trade.hold_days}天")
...
def main():
    engine = LowFreqTradingEngineV11()
    start_date = date(2024, 11, 26)
    end_date = date(2026, 5, 22)

    print(f"\n{'='*70}")
    print(f"低频量化交易系统 v11 (最终优化版)")
    print(f"{'='*70}")
    print(f"回测区间: {start_date} ~ {end_date}")
    print(f"核心优化:")
    print(f"  1. 分批止盈: 盈利>25%时卖出50%")
    print(f"  2. 尾随止损: 盈利>20%后，止损提高到-5%")
    print(f"  3. 移除激进条件，只保留核心")
    print(f"{'='*70}\n")
    ...
    with open(output_dir / f"lowfreq_v11_{start_date.isoformat()}_{end_date.isoformat()}.json", 'w') as f:
