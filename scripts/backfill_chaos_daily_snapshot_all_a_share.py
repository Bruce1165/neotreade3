...
def main() -> int:
    ...
    with sqlite3.connect(str(chaos_db)) as chaos_conn:
        ensure_chaos_schema(chaos_conn)
        for idx, batch_codes in enumerate(batches, start=1):
            if int(args.max_batches) > 0 and int(idx) > int(args.max_batches):
                break
            batch_id = f"{idx:04d}"
            ...
            ready_n, pending_n, other_n = _count_snapshots_for_batch(
                chaos_conn,
                codes=batch_codes,
            ...
            ready_n2, pending_n2, other_n2 = _count_snapshots_for_batch(
                chaos_conn,
                codes=batch_codes,
                start_date=str(start_date),
                end_date=str(end_date),
