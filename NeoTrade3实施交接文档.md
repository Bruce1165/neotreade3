...
> 归档说明：本文生成于 2026-05-26，用于历史会话衔接。文中出现的旧任务名、旧数据入口与旧接口清单仅代表当时系统状态，不代表当前正式生产口径；当前正式日线更新入口已收敛为 `POST /api/data/update`，正式任务名为 `update_daily_prices_authoritative`。

> 2026-06-18 口径纠偏：本文中关于“确定性”的高优先级定位、旧版三维共振表述、以及对低频引擎 `v16` 的能力描述，均只代表当时历史会话上下文，不代表当前正式对齐依据。当前正式业务依据应以以下四层为准：1）用户给出的五维模型调整原始依据；2）后续两份补充修正材料；3）随后口头确认的执行约束（尤其是回撤止损 `-5%` 是硬性指标、`确定性 > 85` 仅作参考）；4）政策指导材料及团队对其重点范围的补充修正。当前正式重点范围已收敛到“第二大类硬科技的细分赛道龙头，尤其是 AI 龙头”，并以 `ETF 高配 + 机构高配` 作为关键增强证据。
...
### 1.0 2026-06-18 当前正式业务依据补充

- 当前正式模型调整依据，不再只围绕“确定性”单一概念展开，而是以上游五维框架为总依据：
  - 周期位置
  - 主流指数择时
  - 行业渗透率
  - 技术层面
  - 资金与情绪面
- 该五维框架的总前提是 `K 型经济`，并要求先识别 `K 型向上 / 向下`，再决定主线和候选范围。
- 后续补充材料对重点范围做了进一步收敛：
  - 第一、第三、第四、第五大类大多属于 `K 型向下` 或低优先级观察范围；
  - 真正的重点范围是第二大类硬科技；
  - 第二大类必须买龙头，而且必须高度关注细分赛道龙头；
  - `ETF 高配 + 机构高配` 是关键增强证据；
  - 人工智能龙头是优先级最高的重点方向。
- 当前执行约束以以下两条为高优先级：
  - 回撤止损 `-5%` 是硬性指标，必须刚性落实；
  - `确定性 > 85` 只作为参考项，不再作为是否对齐的硬门槛。
- 因此，本文下文中凡是把“确定性”写成首要硬约束、或把旧 `v16` 能力完整等同于当前正式模型逻辑的段落，都应按本节补充说明理解。
...
> 2026-06-18 补充说明：当前正式口径下，“确定性”仍可作为解释与参考字段保留，但不再作为分析层、筛选层、交易层是否对齐的首要硬门槛；若与五维框架、重点范围收敛逻辑或 `-5%` 回撤止损这一硬性指标冲突，应以后者为准。
...
> 2026-06-18 补充说明：当前正式优先级中，“尾随止损-5% / 回撤止损-5%”已被再次明确为硬性约束；而“市值范围 200-400 亿”“确定性优先”等旧口径，若与调整后的模型逻辑冲突，应为当前正式逻辑让步。
...
> 2026-06-18 纠偏：上表仅代表历史会话里对 `v16` 的能力描述，不代表当前正式代码事实。当前正式 `lowfreq_engine_v16_advanced.py` 的卖出主链并未完整执行 `partial_profit / target_reached / max_hold`；正式逻辑核对与缺陷分析应以当前代码与回测产物为准，而不能直接沿用此处历史描述。
...
│   ├── data_sources/              # 外部数据源适配器（巨潮/mootdx等）
│   ├── issue_center/              # 问题聚合中心
│   ├── scheduler/                 # 任务调度器
...
├── docs/archive/handover/HANDOVER_DETAILED.md  # 历史交接文档（2026-05-25，已被本文档取代）
...
2. **5-25（周一）日线数据缺失** — 腾讯接口只能获取最近交易日数据，无法补采历史日期；2.0数据源只到5-21；原东方财富补采方案已退役
...
11. **`docs/archive/handover/HANDOVER_DETAILED.md` 与本文档有信息不一致** — 端口信息等存在矛盾，以本文档为准
...
当前优先级：
1. 重新确认 2026-05-25 缺失日线数据的补采方案（原东方财富脚本已退役）
